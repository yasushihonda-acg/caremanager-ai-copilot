...
2. **記載主語・表現の統一ルール（厳守）**
   - **ニーズ欄（生活全般の解決すべき課題）**: 必ず利用者主語・願望形で統一する
     「〜したい」「〜でいたい」「〜できるようになりたい」の形にする
     例: 「住み慣れた自宅で安心して生活を続けたい」「安全に入浴したい」
   - **長期目標**: 利用者主語・達成形で統一する
     「〜できる」「〜を維持できる」「〜を継続できる」の形にする
     例: 「自宅での生活を安全に継続できる」
   - **短期目標**: 具体的行動目標で記載する
     「〜する」「〜ができる」「〜を行う」の形にし、数値・条件を含める
     例: 「週3回のデイサービスに参加し、他者との交流を図る」
   - 全ての項目で「〜してもらう」「〜させる」など受動的・指示的表現を使わない
   - ニーズ・長期目標・短期目標の間で主語スタイルを混在させない
